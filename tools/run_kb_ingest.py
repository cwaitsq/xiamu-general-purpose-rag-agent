...
def main() -> int:
    parser = argparse.ArgumentParser(description="执行知识入库，生成切片并按需刷新 qmd。")
    ...
    parser.add_argument("--no-refresh-index", action="store_true", help="生成切片，但不刷新 qmd。")
    parser.add_argument("--prepare-raw", action="store_true", help="入库前先把 raw 资料整理成标准知识文档。")
    parser.add_argument(
        "--publish-status",
    ...
    if args.validate_only and args.prepare_raw:
        parser.error("--validate-only 不能和 --prepare-raw 一起用。只做整理校验请改用 run_kb_prepare.py。")
